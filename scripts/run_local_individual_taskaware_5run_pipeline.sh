...
usage() {
  cat <<'USAGE'
Usage: run_local_individual_taskaware_5run_pipeline.sh [options]
...
if [[ -z "$PIPE_ID" ]]; then
  PIPE_ID="5multiconfig_taskaware_$(date +%Y%m%d_%H%M%S)_$RANDOM"
fi
...
RUN_TAGS=(ta_rho05 ta_rho0 ta_rglobal2 ta_participation_mid ta_beta50)
declare -A CFG_PREFIXES=(
  [ta_rho05]="individual_federated_sharded_ta_rho05"
  [ta_rho0]="individual_federated_sharded_ta_rho0"
  [ta_rglobal2]="individual_federated_sharded_ta_rglobal2"
  [ta_participation_mid]="individual_federated_sharded_ta_participation_mid"
  [ta_beta50]="individual_federated_sharded_ta_beta50"
)
