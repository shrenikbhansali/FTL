...
CFG_PREFIX="${INDIVIDUAL_CFG_PREFIX:-individual_federated_long}"
CONDA_ENV=""
HF_HOME_OVERRIDE=""

usage() {
  cat <<'USAGE'
Usage: run_local_individual_long_pipeline.sh [options]
...
Options:
  ...
  --cfg-prefix     YAML prefix under yamls/ (default: individual_federated_long).
...
if [[ -z "$PIPE_ID" ]]; then
  PIPE_ID="individual_long_$(date +%Y%m%d_%H%M%S)_$RANDOM"
fi
if [[ -z "$EVAL_ID" ]]; then
