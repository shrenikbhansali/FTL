...
PIPE_ID="${INDIVIDUAL_ALLPAIRS_PIPE_ID:-}"
EVAL_ID="${INDIVIDUAL_ALLPAIRS_EVAL_ID:-}"
...
usage() {
  cat <<'USAGE'
Usage: run_local_individual_allpairs_eval_pipeline.sh [options]
...
if [[ -z "$PIPE_ID" ]]; then
  PIPE_ID="individual_allpairs_$(date +%Y%m%d_%H%M%S)_$RANDOM"
fi
if [[ -z "$EVAL_ID" ]]; then
...
if [[ ${#CLIENT_NAMES[@]} -eq 0 ]]; then
  log "No clients found in manifest: $MANIFEST_PATH"
  exit 1
fi
...
TASKS=(hellaswag xsum hotpotqa mbpp gsm8k)
declare -A COLLECT_EXPS=()

log "Launching all-pairs client evals (gsm8k queued last)..."
for exp in "${EXPS[@]}"; do
  for client_name in "${CLIENT_NAMES[@]}"; do
    client_id="${CLIENT_IDS[$client_name]}"
    ckpt_path="$(client_ckpt_path "$BASELINE_DIR" "$exp" "$client_id")"
    if [[ ! -f "$ckpt_path" ]]; then
      log "Missing client checkpoint: $ckpt_path"
      continue
    fi
    exp_tag="allpairs_${exp}_${client_name}"
    for task in "${TASKS[@]}"; do
      schedule_eval "$exp_tag" "$task" "$ckpt_path"
      COLLECT_EXPS["$exp_tag"]=1
    done
  done
done

log "Waiting for all-pairs evals..."
FAILURES=0
...
if [[ $SKIP_COLLECT -eq 0 && ${#COLLECT_EXPS[@]} -gt 0 ]]; then
  log "Collecting all-pairs metrics..."
...
log "All-pairs client eval pipeline complete."
if [[ $DRY_RUN -eq 1 ]]; then
