...
usage() {
  cat <<'USAGE'
Usage: run_local_individual_taskaware_2run_pipeline.sh [options]

Options:
  --dry-run        Print commands without running them.
  --skip-train     Skip training jobs (training only script).
  --resume         Skip training if checkpoint exists.
  --gpus           Comma-separated GPU list (default: 2,3).
...
if [[ -z "$PIPE_ID" ]]; then
  PIPE_ID="2run_taskaware_$(date +%Y%m%d_%H%M%S)_$RANDOM"
fi

BASE_DIR="$ROOT_DIR/2multiconfig"
...
record_status() {
  ...
  printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
    "$phase" "$label" "$gpu" "$status" "$start_ts" "$end_ts" "$log_file" >> "$STATUS_FILE"
}
...
wait_jobs() {
  ...
  for idx in "${completed_indices[@]}"; do
    unset 'JOB_PIDS[idx]'
...
RUN_TAGS=(ta_mid_shared_strong ta_low_shared_weighted)
declare -A CFG_PREFIXES=(
  [ta_mid_shared_strong]="individual_federated_sharded_ta_mid_shared_strong"
  [ta_low_shared_weighted]="individual_federated_sharded_ta_low_shared_weighted"
)
...
if [[ $SKIP_TRAIN -eq 0 ]]; then
  log "Launching 2‑run taskaware training jobs..."
  ...
  if ! wait_jobs "train"; then
    log "Training finished with failures. See $STATUS_FILE for details."
    exit 1
  fi
else
...
log "2‑run taskaware training complete."
