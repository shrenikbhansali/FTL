...
log "Queuing llama global eval (arrays 0-3) after baseline completion..."
sbatch --parsable --dependency=afterok:${BASELINES_JOB} --array=0-3 "$SBATCH_DIR/sbatch_eval_global_full.sbatch"

log "Queuing llama client eval (arrays 0-11) after baseline completion..."
sbatch --parsable --dependency=afterok:${BASELINES_JOB} --array=0-11 "$SBATCH_DIR/sbatch_eval_clients_full.sbatch"
