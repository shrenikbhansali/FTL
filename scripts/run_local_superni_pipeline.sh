...
usage() {
  cat <<'USAGE'
Usage: run_local_superni_pipeline.sh [options]
...
Options:
  ...
  --eval-id        Override evaluation id (default: eval_<pipe-id>).
...
if [[ -z "$PIPE_ID" ]]; then
  PIPE_ID="superni_local_$(date +%Y%m%d_%H%M%S)_$RANDOM"
fi
if [[ -z "$EVAL_ID" ]]; then
...
export SUPERNI_PIPE_ID="$PIPE_ID"
export SUPERNI_RESULTS_DIR="$RESULTS_DIR"
export SUPERNI_LOGS_DIR="$LOG_DIR"
...
if [[ $SKIP_TRAIN -eq 0 ]]; then
  log "Launching SuperNI training jobs..."
  for exp in "${EXPS[@]}"; do
    ...
    CKPT_PATH="$RESULTS_DIR/$exp/train/ckpt.ckpt"
    OUTDIR="$RESULTS_DIR/$exp/train"
    mkdir -p "$OUTDIR"
    TRAIN_LOG="$LOG_DIR/train_${exp}.log"
    CFG="yamls/superni_federated_${exp}.yaml"
...
if [[ $SKIP_EVAL -eq 0 ]]; then
  log "Waiting for SuperNI evaluations..."
  if ! wait_jobs "eval"; then
    FAILURES=$((FAILURES + 1))
  fi

  log "Collecting SuperNI metrics..."
  for exp in "${EXPS[@]}"; do
...
log "SuperNI local pipeline complete."
if [[ $DRY_RUN -eq 1 ]]; then
