...
PIPE_ID="${INDIVIDUAL_SINGLE_TASK_PIPE_ID:-}"
EVAL_ID="${INDIVIDUAL_SINGLE_TASK_EVAL_ID:-}"
...
FINAL_ROOT="${INDIVIDUAL_FINAL_ROOT:-final}"
...
usage() {
  cat <<'USAGE'
Usage: run_local_individual_single_task_baselines.sh [options]

Options:
  --dry-run             Print commands without running them.
  --resume              Skip steps with existing outputs.
  --skip-train           Skip training jobs.
  --skip-eval            Skip evaluation jobs.
  --skip-collect         Skip metrics aggregation.
  --refresh-configs      Rebuild final config snapshots.
  ...
  --final-root            Final output root (default: final/).
...
if [[ "$DATA_ROOT" == FTL/* ]]; then
  DATA_ROOT="${DATA_ROOT#FTL/}"
fi
if [[ "$DATA_ROOT" != /* ]]; then
...
if [[ "$FINAL_ROOT" == FTL/* ]]; then
  FINAL_ROOT="${FINAL_ROOT#FTL/}"
fi
if [[ "$FINAL_ROOT" != /* ]]; then
  FINAL_ROOT="$ROOT_DIR/$FINAL_ROOT"
fi

if [[ -z "$PIPE_ID" ]]; then
  PIPE_ID="individual_single_task_$(date +%Y%m%d_%H%M%S)_$RANDOM"
fi
if [[ -z "$EVAL_ID" ]]; then
  EVAL_ID="eval_${PIPE_ID}"
fi

FINAL_CONFIG_DIR="$FINAL_ROOT/configs/single_client"
RESULTS_ROOT="$FINAL_ROOT/results/single_client"
TRAIN_ROOT="$RESULTS_ROOT/centralized_single"
LOG_DIR="$FINAL_ROOT/logs/single_client/$PIPE_ID"
PIPE_LOG="$LOG_DIR/pipeline.log"
STATUS_FILE="$LOG_DIR/status.tsv"
FINAL_DATA_DIR="$FINAL_ROOT/data/eval_outputs"

mkdir -p "$FINAL_CONFIG_DIR" "$RESULTS_ROOT" "$TRAIN_ROOT" "$LOG_DIR" "$FINAL_DATA_DIR"
: > "$PIPE_LOG"
printf "phase\tlabel\tgpu\tstatus\tstart\tend\tlog\n" > "$STATUS_FILE"

if [[ ! -f "$CENTRALIZED_CFG" ]]; then
  echo "Centralized config not found: $CENTRALIZED_CFG" >&2
  exit 1
fi
...
schedule_eval() {
  ...
  local res_dir="$RESULTS_ROOT/global/$exp_tag/$task/$EVAL_ID"
  ...
  run_job "eval" "eval-$exp_tag-$task" "$eval_log" \
    bash -c "TMP_BASE='$FINAL_DATA_DIR'; \
...
write_config_snapshot() {
  ...
  local client_name="$2"
  local out_path="$3"
  if [[ -f "$out_path" && $REFRESH_CONFIGS -eq 0 ]]; then
    return 0
  fi
  python3 - "$CENTRALIZED_CFG" "$out_path" "$client_name" <<'PY'
import sys
...
if needle not in text:
    raise SystemExit(f"Expected '{needle}' in {base_path}")
text = text.replace(needle, f'clients: ["{client}"]', 1)
out_path.write_text(text, encoding="utf-8")
PY
  log "Wrote config snapshot: $out_path"
}
...
for dataset in "${DATASETS[@]}"; do
  ...
  if [[ -z "${CLIENT_NAMES[$client_name]:-}" ]]; then
    log "Client not found in manifest: $client_name"
    exit 1
  fi
  cfg_out="$FINAL_CONFIG_DIR/centralized_single_${dataset}.yaml"
  write_config_snapshot "$dataset" "$client_name" "$cfg_out"
done
...
if [[ $SKIP_TRAIN -eq 0 ]]; then
  log "Launching single-task centralized training jobs..."
  for dataset in "${DATASETS[@]}"; do
    cfg_path="$FINAL_CONFIG_DIR/centralized_single_${dataset}.yaml"
    outdir="$TRAIN_ROOT/$dataset/train"
    TRAIN_DIRS["$dataset"]="$outdir"
    if [[ $RESUME -eq 1 ]]; then
  ...
  if [[ $train_jobs -gt 0 ]]; then
    log "Waiting for centralized training jobs..."
  ...
  for dataset in "${DATASETS[@]}"; do
    TRAIN_DIRS["$dataset"]="$TRAIN_ROOT/$dataset/train"
  done
fi

if [[ $SKIP_EVAL -eq 0 ]]; then
  log "Launching single-task evals (matching benchmark only)..."
  for dataset in "${DATASETS[@]}"; do
    train_dir="${TRAIN_DIRS[$dataset]}"
    ckpt_path="$(find_ckpt_in_dir "$train_dir")"
    if [[ -z "$ckpt_path" ]]; then
      log "No checkpoint found for $dataset under $train_dir"
      continue
    fi
  ...
  log "Waiting for centralized evals..."
  if ! wait_jobs "eval"; then
    FAILURES=$((FAILURES + 1))
  fi
fi

if [[ $SKIP_COLLECT -eq 0 && $SKIP_EVAL -eq 0 ]]; then
  log "Collecting centralized single-task metrics..."
  for dataset in "${DATASETS[@]}"; do
    exp_tag="centralized_single_${dataset}"
    collect_log="$LOG_DIR/collect_${exp_tag}.log"
    if [[ $DRY_RUN -eq 1 ]]; then
      log "DRY-RUN [collect-$exp_tag] python scripts/collect_tulu_eval_results.py --results-root $RESULTS_ROOT --exp $exp_tag --eval-job-id $EVAL_ID"
      continue
    fi
    : > "$collect_log"
...
log "Single-task centralized baseline pipeline complete."
if [[ $DRY_RUN -eq 1 ]]; then
