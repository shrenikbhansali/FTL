...
CFG_PREFIX="${INDIVIDUAL_CFG_PREFIX:-individual_federated_sharded}"
CONDA_ENV=""
HF_HOME_OVERRIDE=""

usage() {
  cat <<'USAGE'
Usage: run_local_individual_sharded_pipeline.sh [options]
...
Options:
  ...
  --cfg-prefix     YAML prefix under yamls/ (default: individual_federated_sharded).
...
if [[ -z "$PIPE_ID" ]]; then
  PIPE_ID="individual_sharded_$(date +%Y%m%d_%H%M%S)_$RANDOM"
fi
if [[ -z "$EVAL_ID" ]]; then
  EVAL_ID="eval_${PIPE_ID}"
fi

LOG_DIR="$ROOT_DIR/individual_sharded_logs/$PIPE_ID"
RESULTS_DIR="$ROOT_DIR/individual_sharded_results/$PIPE_ID"
...
export INDIVIDUAL_PIPE_ID="$PIPE_ID"
export INDIVIDUAL_RESULTS_DIR="$RESULTS_DIR"
export INDIVIDUAL_LOGS_DIR="$LOG_DIR"
...
enqueue_eval_jobs() {
  ...
  for task in "${TASKS[@]}"; do
    local res_dir="$RESULTS_DIR/global/$exp/$task/$EVAL_ID"
...
if [[ $SKIP_TRAIN -eq 0 ]]; then
  log "Launching individual federated training jobs..."
  for exp in "${EXPS[@]}"; do
...
if [[ $SKIP_EVAL -eq 0 ]]; then
  log "Waiting for individual evaluations..."
  if ! wait_jobs "eval"; then
    FAILURES=$((FAILURES + 1))
  fi

  log "Collecting individual metrics..."
  for exp in "${EXPS[@]}"; do
...
log "Individual federated pipeline complete."
if [[ $DRY_RUN -eq 1 ]]; then
