...
usage() {
  cat <<'USAGE'
Usage: run_local_individual_pipeline.sh [options]
...
Options:
  ...
  --cfg-prefix     YAML prefix under yamls/ (default: individual_federated).
...
if [[ -z "$PIPE_ID" ]]; then
  PIPE_ID="individual_local_$(date +%Y%m%d_%H%M%S)_$RANDOM"
fi
if [[ -z "$EVAL_ID" ]]; then
