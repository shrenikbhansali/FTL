...
usage() {
  cat <<'USAGE'
Usage: run_ablation_projection_specialization_eval_h200.sh [options]

Evaluate specialization retention (per-client checkpoint on its own dataset)
for projection ablations on a 4x H200 server.

Defaults:
  - tasks: hellaswag,xsum,hotpotqa,mbpp (excludes gsm8k, piqa, APPS)
...
Options:
  ...
  --tasks LIST             Comma-separated task list (default: hellaswag,xsum,hotpotqa,mbpp)
  --exps LIST              Comma-separated ablation names (default: abl_server_only,abl_full)
...
GPU_LIST="0,1,2,3"
TASK_LIST="hellaswag,xsum,hotpotqa,mbpp"
EXP_LIST="abl_server_only,abl_full"
...
if [[ -z "$PIPE_ID" ]]; then
  PIPE_ID="spec_projection_h200_$(date +%Y%m%d_%H%M%S)_$RANDOM"
fi
...
IFS=',' read -r -a DATASETS <<< "$TASK_LIST"
IFS=',' read -r -a EXPS <<< "$EXP_LIST"
